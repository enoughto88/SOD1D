...
subroutine Euler1D(NXMAX,DX,DT,GAMMA,rho,vel,pre,cfl)
   ...
   call AUSMPlus(NXMAX,GAMMA,umuscl,flux)
...
subroutine AUSMPlus(NXMAX,GAMMA,umuscl,flux)
   ...
   do i = 1,NXMAX+1
      if(dabs(Mach(i,1)).ge.1.0d0) then
         ...
         Mplus(i)  = 0.25d0*(Mach(i,1)+1.0d0)**2.0d0+1.0d0/8.0d0*(Mach(i,1)**2.0d0-1.0d0)**2.0d0
      endif
      if(dabs(Mach(i,2)).ge.1.0d0) then
         Mminus(i) = 0.5d0*(Mach(i,2)-dabs(Mach(i,2)))
      else
         Mminus(i) =-0.25d0*(Mach(i,2)-1.0d0)**2.0d0-1.0d0/8.0d00*(Mach(i,2)**2.0d0-1.0d0)**2.0d0
      endif
      mborder(i)  = Mplus(i)+Mminus(i)
      machpm(i,1) = 0.5d0*(mborder(i)+dabs(mborder(i)))
      machpm(i,2) = 0.5d0*(mborder(i)-dabs(mborder(i)))
   ...
   do i = 1,NXMAX+1
      if(dabs(Mach(i,1)).ge.1.0d0) then
         ...
         prespm(i,1) = 0.25d0*(Mach(i,1)+1.0d0)**2.0d0*(2.0d0-Mach(i,1))+3.0d0/16.0d0*Mach(i,1)*(Mach(i,1)**2.0d0-1.0d0)**2.0d0
      endif
      if(dabs(Mach(i,2)).ge.1.0d0) then
         prespm(i,2) = 0.5d0*(1.0d0-dsign(1.0d0,Mach(i,2)))
      else
         prespm(i,2) = 0.25d0*(Mach(i,2)-1.0d0)**2.0d0*(2.0d0+Mach(i,2))-3.0d0/16.0d0*Mach(i,2)*(Mach(i,2)**2.0d0-1.0d0)**2.0d0
      endif
   enddo
